...
# default
engine = create_engine('mysql://{}:{}@{}:{}/{}'.format(USER, PASSWORD, HOST, PORT, DBNAME))
# meta = MetaData()
# meta.reflect(bind=engine)
conn = engine.connect()
# conn.execute(meta.tables['pages'].insert().values(url='http://geekbrains.ru/', site_id='1'))
...
class CrawlerSitesConnector:
    ...
    def save_stack(self, data):
        # found_time - date object
        try:
            conn.execute('''INSERT INTO pages(url, site_id, found_date_time)
                            VALUES(%s, %s, %s)
                         ''', data)
            # CURSOR.executemany('''
            # INSERT INTO pages(url, site_id, found_date_time)
